...
namespace NavigationTests 
{
    ...
    public:
        ...
        TEST_METHOD(CorrectlyBuildsNavigationPath_Dalaran)
        {
            const auto path = GetNavigationDllPath();
            HINSTANCE hGetProcIDDLL = LoadLibrary(path.c_str());
            ...
            int length = 0;
            XYZ start = XYZ{ 5819.43, 565.11, 650.25 };
            XYZ end = XYZ{ 5838.89, 589.66, 650.6 };
            XYZ* navPath = calculatePathFunc(571, start, end, false, &length);
